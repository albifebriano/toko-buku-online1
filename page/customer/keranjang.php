...
if($aksi=="hapus"){
	...
	$query_buku = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * from buku where id_buku='$id_buku'");
	$data_buku = mysqli_fetch_array($query_buku);
	$stok = $data_buku['stok'];
	$stok_ubah = $stok+$qty;
	mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE buku set stok='$stok_ubah' where id_buku='$id_buku'");
...
<table class="table table-bordered">
	...
	<?php while($keranjang=mysqli_fetch_array($qry)){?>
		<tr>
		<td><?php
		$q = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * from buku where id_buku='$keranjang[id_buku]'");
